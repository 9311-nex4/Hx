...
function NhanVat.CopyAppearance(sourceModel, targetModel)
	...
	if sHum and tHum then
		...
		if ok and desc then
			...
			if applyOk then
				AnimCache[targetModel] = nil -- Xóa cache animation cũ sau khi biến hình
...
function NhanVat.LoadUserAppearance(model, userId)
	...
	if hum then
		...
		if applyOk then
			task.wait()
			AnimCache[model] = nil -- Xóa cache animation cũ sau khi biến hình
			model:ScaleTo(1)
...
function NhanVat.LoadOutfitAppearance(model, outfitId)
	model = model or State.CurrentDummy
	if not model then ThongBaoLoi("Lỗi", "Chưa có Mẫu!") return end
	...
	if not ok or not desc then
		ThongBaoLoi("Lỗi", "Không thể lấy body từ Outfit ID " .. tostring(outfitId))
		return
	end
	...
	if hum then
		...
		if applyOk then
			...
			model:ScaleTo(1)
			State.CurrentScale = 1
			ThongBao("Ngoại hình", "Đã tải body từ Outfit ID: " .. tostring(outfitId))
		else
			ThongBaoLoi("Lỗi", "Không thể áp dụng body!")
		end
	else
		ThongBaoLoi("Lỗi", "Model không có Humanoid!")
	end
...
local function AttachAccessory(targetModel, id, addBtn)
	...
	local result = nil
	local ok, info = pcall(MarketplaceService.GetProductInfoAsync, MarketplaceService, id)
	if ok and info then
		local typeId = info.AssetTypeId
		local desc = Instance.new("HumanoidDescription")
		local valid = true

		if typeId==8 then desc.HatAccessory = tostring(id)
		elseif typeId==41 then desc.HairAccessory = tostring(id)
		elseif typeId==42 then desc.FaceAccessory = tostring(id)
		elseif typeId==43 then desc.NeckAccessory = tostring(id)
		elseif typeId==44 then desc.ShouldersAccessory = tostring(id)
		elseif typeId==45 then desc.FrontAccessory = tostring(id)
		elseif typeId==46 then desc.BackAccessory = tostring(id)
		elseif typeId==47 then desc.WaistAccessory = tostring(id)
		elseif typeId==11 then desc.Shirt = id
		elseif typeId==12 then desc.Pants = id
		elseif typeId==2 then desc.GraphicTShirt = id
		else valid = false end

		if valid then
			local s, tmp = pcall(Players.CreateHumanoidModelFromDescriptionAsync, Players, desc, Enum.HumanoidRigType.R6)
			if s and tmp then
				for _, v in ipairs(tmp:GetChildren()) do
					if v:IsA("Accessory") or v:IsA("Clothing") or v:IsA("ShirtGraphic") then result = v:Clone() break end
				end
				tmp:Destroy()
			end
		end
	...
	if not result then pcall(function() result = game:GetObjects("rbxassetid://"..id)[1] end) end
	if not result then
		pcall(function()
...
function NhanVat.OpenCustomMenu(targetModel)
	...
	SectionLabel(bodyScroll, "Tải Body từ Outfit ID", 3)

	local outfitRow = Instance.new("Frame")
	outfitRow.Size = UDim2.new(1, 0, 0, 36)
	outfitRow.BackgroundTransparency = 1
	outfitRow.LayoutOrder = 4
	outfitRow.Parent = bodyScroll

	local outfitBox = Instance.new("TextBox")
	outfitBox.Size = UDim2.new(1, -98, 1, 0)
	outfitBox.BackgroundColor3 = C.NenHop
	outfitBox.PlaceholderText = "Nhập Outfit ID..."
	outfitBox.PlaceholderColor3 = C.ChuMo
	outfitBox.Text = ""
	outfitBox.TextColor3 = C.Chu
	outfitBox.Font = Enum.Font.Gotham
	outfitBox.TextSize = 12
	outfitBox.TextXAlignment = Enum.TextXAlignment.Left
	outfitBox.ClearTextOnFocus = false
	Corner(outfitBox, 6)
	Stroke(outfitBox, C.VienNeon, 0.6)
	Padding(outfitBox, 8)
	outfitBox.Parent = outfitRow

	local loadOutfitBtn = MakeBtn(outfitRow, "Tải", C.NenMuc, UDim2.new(0, 80, 1, 0), UDim2.new(1, -88, 0, 0))
	Stroke(loadOutfitBtn, C.VienNeon, 0.6)
	Ripple(loadOutfitBtn)
	loadOutfitBtn.MouseButton1Click:Connect(function()
		local oid = tonumber(outfitBox.Text:match("%d+"))
		if not oid then ThongBaoLoi("Lỗi", "Outfit ID không hợp lệ!") return end
		loadOutfitBtn.Text = "..."
		task.spawn(function()
			NhanVat.LoadOutfitAppearance(targetModel, oid)
			loadOutfitBtn.Text = "Tải"
			RefreshList()
		end)
	end)

	SectionLabel(bodyScroll, "Tỉ lệ nhân vật", 5)

	scaleRow.LayoutOrder = 6
	...
	SectionLabel(bodyScroll, "Màu sắc cơ thể", 7)
	local presetColors = {
