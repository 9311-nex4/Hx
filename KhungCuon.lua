...
local function TaoHieuUng(NutBam, NoiDung)
	...
	local propsLuot, propsGoc = {BackgroundColor3 = MauLuot}, {BackgroundColor3 = MauGoc}
...
function ThanhPhan.Otich(Cha, DuLieu, CauHinh, CapNhat, DeQuy)
	...
	local NutTich = TaoDoiTuong("TextButton", {Name = "NutTich", LayoutOrder = 1, Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = DuLieu.MauNenRieng or Mau.NenMuc, Text = "", AutoButtonColor = false, ZIndex = 2, Parent = HangNgang})
	TaoBoGoc(NutTich, 8) TaoVien(NutTich, Mau.VienNeon, 0.6)

	local DoDaiChuoi = utf8.len(DuLieu.Ten) or string.len(DuLieu.Ten)
	if DoDaiChuoi > 10 then
		local VungCuonChu = TaoDoiTuong("ScrollingFrame", {Size = UDim2.new(0.72, -5, 1, 0), Position = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1, ScrollBarThickness = 0, ScrollingDirection = Enum.ScrollingDirection.X, AutomaticCanvasSize = Enum.AutomaticSize.X, CanvasSize = UDim2.new(0,0,0,0), ClipsDescendants = true, ZIndex = 3, Parent = NutTich})
		local NhanTich = TaoNhan({Text = "  " .. DuLieu.Ten, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = Enum.AutomaticSize.X, TextColor3 = Mau.Chu, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 3, Parent = VungCuonChu})
		NhanTich.TextScaled = false NhanTich.TextWrapped = false NhanTich.TextSize = CauHinh.VanBan.Nho + 2
	else
		local NhanTich = TaoNhan({Text = "  " .. DuLieu.Ten, Size = UDim2.new(0.72, -5, 1, 0), TextColor3 = Mau.Chu, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true, ZIndex = 3, Parent = NutTich})
		TaoGioiHanChu(NhanTich, CauHinh.VanBan.Nho + 2)
	end

	local HopVuong = TaoDoiTuong("Frame", {Size = UDim2.fromOffset(26, 26), Position = UDim2.new(1, -12, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Mau.NenHop, ZIndex = 3, Parent = NutTich})
	TaoBoGoc(HopVuong, 6) TaoVien(HopVuong, Mau.VienNeon, 0.7)

	DuLieu.TrangThai = (DuLieu.HienTai == "Bat") or DuLieu.TrangThai or false
	local DauTich = TaoDoiTuong("Frame", {Size = UDim2.new(1, -8, 1, -8), Position = UDim2.fromScale(0.5, 0.5), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Mau.TichBat, Visible = DuLieu.TrangThai, ZIndex = 4, Parent = HopVuong})
	TaoBoGoc(DauTich, 4)
	...
	if DuLieu.LoaiNutCon == "CungHang" then
		NutPhu = TaoDoiTuong("TextButton", {LayoutOrder = 2, Size = UDim2.new(0.3, 0, 1, 0), BackgroundColor3 = DuLieu.MauNenRieng or Mau.NenMuc, Text = "...", TextColor3 = Mau.Chu, Font = "GothamMedium", TextScaled = true, Visible = false, ZIndex = 2, Parent = HangNgang})
		TaoBoGoc(NutPhu, 8) TaoVien(NutPhu, Mau.VienNeon, 0.7) TaoGioiHanChu(NutPhu, CauHinh.VanBan.Nho + 2)
	...
	local function LamMoi()
		...
		if DuLieu.TrangThai and DuLieu.CacNutCon and #DuLieu.CacNutCon > 0 then
			...
			for ThuTuNho, DuLieuNho in ipairs(DuLieu.CacNutCon) do 
				DuLieuNho.MauNenRieng = Mau.NenMuc DeQuy(KhungChuaPhu, DuLieuNho, CapNhat, ThuTuNho) 
			end
		else
...
function ThanhPhan.Nut(Cha, DuLieu, CauHinh, CapNhat, DeQuy)
	local Mau = CauHinh.Mau
	local NutBam = TaoDoiTuong("TextButton", {LayoutOrder = DuLieu.ThuTu, Size = UDim2.new(1, 0, 0, 45), BackgroundColor3 = DuLieu.MauNenRieng or Mau.NenMuc, Text = DuLieu.Ten, TextColor3 = Mau.Chu, Font = "GothamMedium", TextScaled = true, ZIndex = 2, Parent = Cha})
	TaoBoGoc(NutBam, 8) TaoVien(NutBam, Mau.VienNeon, 0.6) TaoDoiTuong("UIPadding", {PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 6), Parent = NutBam}) TaoGioiHanChu(NutBam, CauHinh.VanBan.Nut + 2) TaoHieuUng(NutBam, NutBam)
...
function ThanhPhan.NhieuNut(Cha, DuLieu, CauHinh, CapNhat)
	local Mau = CauHinh.Mau
	local KhungChua = TaoDoiTuong("Frame", {LayoutOrder = DuLieu.ThuTu, Size = UDim2.new(1, 0, 0, 45), BackgroundTransparency = 1, Parent = Cha}, {TaoDoiTuong("UIListLayout", {FillDirection = "Horizontal", SortOrder = "LayoutOrder", Padding = UDim.new(0, 8)})})

	local function TaoNutNho(TenNut, SuKienNut, ThuTuNut, TongSoNut)
		local DoRong = 1 / (TongSoNut or 1)
		local NutBam = TaoDoiTuong("TextButton", {LayoutOrder = ThuTuNut, Size = UDim2.new(DoRong, -((8 * (TongSoNut - 1)) / TongSoNut), 1, 0), BackgroundColor3 = DuLieu.MauNenRieng or Mau.NenMuc, Text = TenNut or "Nut", TextColor3 = Mau.Chu, Font = "GothamMedium", TextScaled = true, ZIndex = 2, Parent = KhungChua})
		TaoBoGoc(NutBam, 8) TaoVien(NutBam, Mau.VienNeon, 0.6) TaoDoiTuong("UIPadding", {PaddingLeft = UDim.new(0, 4), PaddingRight = UDim.new(0, 4), Parent = NutBam}) TaoGioiHanChu(NutBam, CauHinh.VanBan.Nho + 2) TaoHieuUng(NutBam, NutBam)
...
function ThanhPhan.HopXo(Cha, DuLieu, CauHinh, CapNhat, DeQuy, NguCanh)
	...
	TaoBoGoc(HangNgang, 8) TaoVien(HangNgang, Mau.VienNeon, 0.5)
	...
	local HienThi = TaoDoiTuong("TextButton", {Size = UDim2.new(0.48, 0, 0, 32), Position = UDim2.new(1, -6, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Mau.NenHop, BackgroundTransparency = 0.5, Text = DuLieu.HienTai or "...", TextColor3 = Mau.Chu, Font = "Gotham", TextScaled = true, ZIndex = 3, Parent = HangNgang})
	TaoBoGoc(HienThi, 6) TaoVien(HienThi, Mau.VienNeon, 0.8) TaoGioiHanChu(HienThi, CauHinh.VanBan.Nho + 2) TaoHieuUng(HienThi, nil)
	...
	HienThi.MouseButton1Click:Connect(function()
		...
		HopXoDangMo = true
		local KhungXo = TaoDoiTuong("Frame", {Size = UDim2.new(0, HienThi.AbsoluteSize.X, 0, 0), Position = UDim2.fromOffset(HienThi.AbsolutePosition.X, HienThi.AbsolutePosition.Y + HienThi.AbsoluteSize.Y + 6), BackgroundColor3 = Mau.NenPhu, ClipsDescendants = true, BorderSizePixel = 0, ZIndex = 105, Parent = NguCanh and NguCanh.LopPhu})
		...
		for ViTriLuaChon, DuLieuLuaChon in ipairs(DuLieu.LuaChon) do
			...
			local MucLuaChon = TaoDoiTuong("TextButton", {LayoutOrder = ViTriLuaChon, Size = UDim2.new(1, -2, 0, 34), BackgroundColor3 = bgLuaChon, BackgroundTransparency = DuocChon and 0.5 or 1, Text = "  " .. TenLuaChon, TextColor3 = DuocChon and Mau.TichBat or Mau.Chu, Font = DuocChon and "GothamBold" or "Gotham", TextScaled = true, TextXAlignment = "Left", ZIndex = 107, AutoButtonColor = false, Parent = Cuon})
			TaoBoGoc(MucLuaChon, 6) TaoGioiHanChu(MucLuaChon, CauHinh.VanBan.Nho + 2)
...
function ThanhPhan.Danhsach(Cha, DuLieu, CauHinh, CapNhat, DeQuy, NguCanh)
	...
	TaoBoGoc(DauMuc, 8) TaoVien(DauMuc, Mau.VienNeon, 0.6)
	...
	local KhungChua = TaoDoiTuong("Frame", {LayoutOrder = 2, Size = UDim2.new(1, 0, 0, 0), BackgroundColor3 = Mau.NenHop, BackgroundTransparency = 0.4, ClipsDescendants = true, Parent = KhungChinh})
	TaoBoGoc(KhungChua, 6) TaoVien(KhungChua, Mau.VienNeon, 0.9) TaoDoiTuong("UIPadding", {PaddingTop = UDim.new(0, 8), PaddingBottom = UDim.new(0, 8), PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8), Parent = KhungChua})
...
function ThanhPhan.Odien(Cha, DuLieu, CauHinh, CapNhat, DeQuy)
	local Mau = CauHinh.Mau
	local KhungChinh = TaoDoiTuong("Frame", {LayoutOrder = DuLieu.ThuTu, Size = UDim2.new(1, 0, 0, 45), BackgroundTransparency = 1, AutomaticSize = "Y", Parent = Cha}, {TaoDoiTuong("UIListLayout", {SortOrder = "LayoutOrder"})})
	local HangNgang = TaoDoiTuong("Frame", {LayoutOrder = 1, Size = UDim2.new(1, 0, 0, 45), BackgroundColor3 = DuLieu.MauNenRieng or Mau.NenMuc, ZIndex = 2, Parent = KhungChinh})
	TaoBoGoc(HangNgang, 8) TaoVien(HangNgang, Mau.VienNeon, 0.6)
	...
	local ONhap = TaoDoiTuong("TextBox", {Size = UDim2.new(0.35, 0, 0, 32), Position = UDim2.new(1, -6, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Mau.NenHop, BackgroundTransparency = 0.5, Text = tostring(DuLieu.HienTai or ""), PlaceholderText = DuLieu.GoiY or "", TextColor3 = Mau.Chu, Font = "Gotham", TextScaled = true, ZIndex = 3, Parent = HangNgang})
	TaoBoGoc(ONhap, 6) TaoVien(ONhap, Mau.VienNeon, 0.7) TaoGioiHanChu(ONhap, CauHinh.VanBan.Nho + 2)
...
function ThanhPhan.PhimNong(Cha, DuLieu, CauHinh, CapNhat, DeQuy)
	...
	local NutPhim = TaoDoiTuong("TextButton", {Size = UDim2.new(0.35, 0, 0, 32), Position = UDim2.new(1, -6, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Mau.NenHop, BackgroundTransparency = 0.5, Text = tostring(DuLieu.HienTai or "None"), TextColor3 = Mau.Chu, Font = "GothamBold", TextScaled = true, ZIndex = 3, Parent = HangNgang})
	TaoBoGoc(NutPhim, 6) TaoVien(NutPhim, Mau.VienNeon, 0.7) TaoGioiHanChu(NutPhim, CauHinh.VanBan.Nho + 2) TaoHieuUng(NutPhim, nil)
...
local function TaoBoCucCacKhoi(VungCuon, DanhSachKhoiTao, SoCotTab, CauHinh, TaoMuc)
	...
	local function TaoKhoi(DuLieuKhoi, CotBoTri)
		...
		local function CapNhatKichThuoc() Khoi.Size = UDim2.new(1, 0, 0, DangMoRong and (50 + BoCucThan.AbsoluteContentSize.Y + 18) or 50) end
		...
		DauMuc.MouseButton1Click:Connect(function() 
			...
			ChayTween(Khoi, TweenMuot, {Size = UDim2.new(1, 0, 0, DangMoRong and (50 + BoCucThan.AbsoluteContentSize.Y + 18) or 50)})
...
function KhungCuon.Tao(Cuon, DuLieu, CauHinh, LopPhu, DongMenu)
	...
	if CauHinh.DangTab then
			...
			for _, ThongTin in ipairs(DanhSachTab) do
				if ThongTin.Index == IndexCuaTab then
					...
					if InitialLoad then
						...
						ThongTin.ThanhLine.BackgroundTransparency = 0 ThongTin.ThanhLine.Size = UDim2.new(0.8, 0, 0, 2) ThongTin.ThanhLine.BackgroundColor3 = Mau.TichBat
					else
						ChayTween(ThongTin.Nut, TweenNhanh, tweenTabChon) ChayTween(ThongTin.ThanhLine, TweenNhanh, {BackgroundTransparency = 0, Size = UDim2.new(0.8, 0, 0, 2), BackgroundColor3 = Mau.TichBat})
					end
		...
		for ViTriTab, DuLieuTab in ipairs(DuLieu) do
			local NutTab = TaoDoiTuong("TextButton", {LayoutOrder = ViTriTab, Size = UDim2.new(0, 0, 0, 28), AutomaticSize = Enum.AutomaticSize.X, BackgroundColor3 = Mau.NenHop, BackgroundTransparency = 0.8, Text = DuLieuTab.TenTab or "Tab " .. ViTriTab, TextColor3 = Mau.Chu, TextTransparency = 0.4, Font = Enum.Font.GothamMedium, TextSize = CauHinh.VanBan.Nho + 2, TextWrapped = false, AutoButtonColor = false, Parent = ThanhTab})
			TaoBoGoc(NutTab, 6) TaoDoiTuong("UIPadding", {PaddingLeft = UDim.new(0, 16), PaddingRight = UDim.new(0, 16), Parent = NutTab})
			local ThanhLine = TaoDoiTuong("Frame", {Size = UDim2.new(0.3, 0, 0, 2), Position = UDim2.new(0.5, 0, 1, -1), AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = Mau.Chu, BackgroundTransparency = 0.6, BorderSizePixel = 0, Parent = NutTab})
			TaoBoGoc(ThanhLine, 2)
			local VungCuonRieng = TaoDoiTuong("ScrollingFrame", {Size = UDim2.fromScale(1, 1), BackgroundTransparency = 1, ScrollBarThickness = 4, ScrollBarImageColor3 = Mau.Chu, BorderSizePixel = 0, ScrollingDirection = Enum.ScrollingDirection.Y, AutomaticCanvasSize = Enum.AutomaticSize.Y, Visible = false, Parent = KhungNoiDungTab})
